<script setup>
    ...
    const testSubmissionTableRef        = ref( null );              // Used for exportCSV
    const searchFilter                  = ref( "" );
    const filterTestSubmissionsTable    = ref( props.filters );     // Used for filtering the table

    const viewTestSubmission = ( testSubmission ) => {    
        router.visit(`/test-submissions/${testSubmission.id}`);
    };

    watch(() => searchFilter, () => {
        router.get('/test-submissions/', { search: searchFilter.value }, { preserveState: true, replace: true });
    }, { deep: true });
</script>
...
<template>
    <AppPageWrapper>
        ...
        <template #page-content>
            ...
            <DataTable ref="testSubmissionTableRef" :value="test_submissions" dataKey="id" paginator :rows="5" responsiveLayout="scroll" v-model:filters="filterTestSubmissionsTable">
                ...
                <Column headerStyle="min-width:10rem;">
                    <template #body="slotProps">
                        <Button icon="pi pi-eye" class="mr-2" rounded outlined @click="viewTestSubmission(slotProps.data)" />
                    </template>
                </Column>
            </DataTable>
        </template>
