...
import BaseTNumberInput from '@/Components/Base/BaseNumberInput.vue';
...
<template>
    <AppPageWrapper>
        ...
        <template #page-content>
            <ProgressBar :value="completionProgress" class="mb-4" />

            <form @submit.prevent="submit" autocomplete="off">
                <!-- Title -->
                <BaseTextInput v-model="careerData.title" label="Τίτλος Επαγγέλματος"
                    placeholder="Προγραμματιστής Λογισμικού" :errors="errors.title" />

                <!-- Related Keywords -->
                <BaseTextInput v-model="careerData.keywords" label="Σχετικές Λέξεις - Κλειδιά"
                ...
                <!-- Popular -->
                <BaseToggleSwitch v-model="careerData.isPopular" label="Ορισμός ως Δημοφιλές Επάγγελμα" />

                <!-- Description -->
                <BaseTextareaInput v-model="careerData.description" label="Συνοπτική Περιγραφή" :rows="10"
                    :errors="errors.description" />

                <!-- LINK -->
                <BaseTextInput v-model="careerData.link" label="Σύνδεσμος προς το FutureGeneration (αν υπάρχει)"
                    type="text" :errors="errors.keywords" />
                <div class="my-4">
                    <small class="ml-2">
                        Κάνε click <a
                            :href="`https://www.google.com/search?q=site:futuregeneration.gr+επάγγελμα+${careerData.title}`"
                            target="_blank"><strong>εδώ</strong></a> και να δεις αν υπάρχει σχετικό δημοσιευμένο
                        επάγγελμα.
                    </small>
                </div>
                ...
                <div class="ml-2">
                    ...
                    <template v-for="(resp, index) in careerData.responsibilities" :key="index">
                        <div class="flex">
                            ...
                            <Button @click.prevent="removeResponsibility(index)" icon="pi pi-times" severity="danger"
                                text class="mb-3" v-tooltip.bottom="'Αφαίρεση αρμοδιότητας'" />

                            <div class="w-full">
                                <BaseTextInput v-model="resp.text" :errors="errors.responsibility" />
                            </div>
                        </div>
                    </template>

                    <!-- Career Responsibilities -->
                    <Button @click.prevent="addCareerResposibility" label="Αρμοδιότητας" icon="pi pi-plus" class="mt-2"
                        outlined />
                </div>

                <!-- Δεξιότητες -->
                <BaseMultiselectInput :options="skillDropdownOptions" v-model="careerData.connections.skills"
                    label="Σχετικές Δεξιότητες" :errors="errors.role"></BaseMultiselectInput>

                <!-- Ενδιαφέροντα -->
                <BaseMultiselectInput :options="interestDropdownOptions" v-model="careerData.connections.interests"
                    label="Σχετικά Ενδιαφέροντα" :errors="errors.interests"></BaseMultiselectInput>

                <!-- Μαθήματα -->
                <BaseMultiselectInput :options="courseDropdownOptions" v-model="careerData.connections.courses"
                    label="Σχετικά Μαθήματα" :errors="errors.courses"></BaseMultiselectInput>
                ...
                <!-- <div class="ml-2 my-4">
                    <div class="grid">
                        <div class="col-12">
                            ...
                            <BaseTNumberInput v-model="salary.start" label="Ελάχιστος Μισθός" />
                            <BaseTNumberInput v-model="salary.end" label="Μέγιστος Μισθός" />
                    ...
                    <div class="formgrid grid">
                        <div class="field">
                            <template v-for="code in careerData.hollandCodes" :key="code.id">
                                <BaseToggleSwitch v-model="code.value" :label="code.name" class="w-full" />
                            </template>
                        </div>
                    </div>
                </div>
                <Button @click="submit" label="Αποθήκευση Επαγγέλματος" raised class="mb-2 mr-2" />
            </form>
        </template>
    </AppPageWrapper>
</template>