<template>
    <div class="card h-full">
        ...
        <div class="flex justify-content-between align-items-start mt-3">
            <div class="w-6">
                <span class="text-4xl font-bold text-900">{{ value }}</span>
                <div :class="percentageChangeClass">
                <span class="font-medium">{{ percentageChange }}</span>
                    <i :class="iconClass"></i>
                </div>
            </div>
            <div class="w-6">
...
<script setup>
    ...
    const percentageChangeClass = computed(() => ({
        'text-green-500': props.isPositiveChange,
        'text-red-500': !props.isPositiveChange
    }));

    const iconClass = computed(() => ({
        'pi pi-arrow-up text-xs ml-2': props.isPositiveChange,
        'pi pi-arrow-down text-xs ml-2': !props.isPositiveChange
    }));
</script>
